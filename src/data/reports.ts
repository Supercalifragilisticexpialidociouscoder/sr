...
function vehicleReport({ db, ledger, range, vehicleId }: BuildContext): ReportTable {
  ...
  const rows = vehicles.map((v) => {
    ...
    return {
      ...
      adblue: Math.round(s.byCategory.adblue),
      fastag: Math.round(s.byCategory.fastag + s.byCategory.toll),
      otherCost: Math.round(s.byCategory.insurance + s.byCategory.documents + s.byCategory.other),
  ...
  return {
    ...
    columns: [
      ...
      { key: 'adblue', label: 'AdBlue (INR)', numeric: true },
      { key: 'fastag', label: 'FASTag and toll (INR)', numeric: true },
      { key: 'maintenance', label: 'Maintenance (INR)', numeric: true },
      { key: 'driverCost', label: 'Driver cost (INR)', numeric: true },
      { key: 'otherCost', label: 'Insurance, documents and other (INR)', numeric: true },
    ...
    totals: {
      ...
      adblue: totalOf('adblue'),
      fastag: totalOf('fastag'),
